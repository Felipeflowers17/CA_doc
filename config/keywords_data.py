# --- Listas de Texto para el Scoring ---

# NOTA: Debes llenar estas listas con los datos reales de tu negocio.
# Estos son solo ejemplos.

# Organismos que suman +5 puntos 
ORGANISMOS_PRIORITARIOS = [
    "I MUNICIPALIDAD DE CANELA",
    "Departamento Provincial de Educación Arauco"

    # ... agregar todos los organismos de alto interés aquí ...
]

# Palabras clave en el TÍTULO que suman +2 puntos 
KEYWORDS_TITULO = [
    "flautas",
    "dulces",
    "escuela",
    "filtro",
    "reutilizable",
    "ventilador",
    "avea"

    # ... agregar todas las palabras clave de título aquí ...
]

# Palabras clave en PRODUCTOS SOLICITADOS  que suman +5 puntos (para Fase 2) 
KEYWORDS_PRODUCTOS_ALTO_VALOR = [
    "niples de tuberia",
    "codos de tubo",
    "fitting",
    "valvula",
    # ... agregar todos los productos de alto valor aquí ...
]